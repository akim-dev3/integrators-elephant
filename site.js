(function () {
  console.log('slb: скрипт подключён');
  function slbInit() {
    var SLB = document.getElementById('slb-app');
    if (!SLB) { console.log('slb: блок на этой странице не найден'); return; }
    console.log('slb: инициализация');

const $  = (sel, root = SLB) => root.querySelector(sel);
const $$ = (sel, root = SLB) => [...root.querySelectorAll(sel)];
...
const LITE = LOW_END;
if (LITE) SLB.classList.add('lite');
(() => {
if (LITE) return;
...
if (fps < 40) SLB.classList.add('lite');
};
...
try {
(() => {
SLB.querySelectorAll('.btn').forEach(b => {
...
toast('Заявка принята! Свяжемся за 15 минут', 'ok');
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', slbInit);
  } else {
    slbInit();
  }
})();
